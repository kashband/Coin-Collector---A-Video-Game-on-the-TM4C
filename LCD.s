...
; Last modification date: 3/29/2022
...
checkOpen	
	LDR R4, =SSI0_SR_R
	LDR R5, [R4]
	ANDS R5, #0x010; checks if BUSY bit is low
	BNE checkOpen
	
    LDR R4, =GPIO_PORTA_DATA_R	
	LDR R5, [R4]
	BIC R5, #0x40
	STR R5, [R4]; Clears PA6
	
	LDR R4, =SSI0_DR_R
	STR R0, [R4]; Writes command to SSI0_DR_R
...
checkReady	
	...
	LDR R5, [R4]
	ANDS R5, #0x2
	BEQ checkReady; Checks if bit 1 is low
    ...
    LDR R4, =SSI0_DR_R
	STR R0, [R4]; Writes data to SSI0_DR_R
